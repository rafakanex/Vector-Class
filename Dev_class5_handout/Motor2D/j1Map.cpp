...
bool j1Map::LoadLayer(pugi::xml_node& maplayer_node, MapLayer* layer)
{
	bool ret = true;
	layer->name.create(maplayer_node.attribute("name").as_string());
	layer->width = maplayer_node.attribute("width").as_uint();
	layer->height = maplayer_node.attribute("height").as_uint();
	memset(layer->gid, 0u, sizeof(uint) * layer->width * layer->height);
}